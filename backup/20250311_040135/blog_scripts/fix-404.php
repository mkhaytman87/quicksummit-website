...
if (file_exists($htaccess_path)) {
    ...
    // Check if .htaccess has WordPress rewrite rules
    if (strpos($htaccess_content, 'RewriteEngine On') !== false && 
        strpos($htaccess_content, 'RewriteBase') !== false) {
        log_action(".htaccess contains WordPress rewrite rules.", "success");
    } else {
        log_action(".htaccess does not contain proper WordPress rewrite rules. Attempting to update...", "info");
        
        // Get the rewrite rules from WordPress
        $rules = $wp_rewrite->mod_rewrite_rules();
        
        if (!empty($rules)) {
            // Attempt to update .htaccess
            if (is_writable($htaccess_path)) {
                // Create a new .htaccess with the WordPress rules
                $htaccess_content = "# BEGIN WordPress\n<IfModule mod_rewrite.c>\nRewriteEngine On\nRewriteBase /blog/\nRewriteRule ^index\.php$ - [L]\nRewriteCond %{REQUEST_FILENAME} !-f\nRewriteCond %{REQUEST_FILENAME} !-d\nRewriteRule . /blog/index.php [L]\n</IfModule>\n# END WordPress\n";
                
                if (file_put_contents($htaccess_path, $htaccess_content)) {
                    log_action(".htaccess updated with WordPress rewrite rules.", "success");
                } else {
                    log_action("Failed to update .htaccess. Please check file permissions.", "error");
                }
            } else {
                log_action(".htaccess is not writable. Please update it manually with the following rules:", "error");
                echo "<pre>" . htmlspecialchars($rules) . "</pre>";
            }
        }
    }
} else {
    log_action(".htaccess file not found. Creating it...", "info");
    
    // Create a basic .htaccess file with WordPress rewrite rules
    $htaccess_content = "# BEGIN WordPress\n<IfModule mod_rewrite.c>\nRewriteEngine On\nRewriteBase /blog/\nRewriteRule ^index\.php$ - [L]\nRewriteCond %{REQUEST_FILENAME} !-f\nRewriteCond %{REQUEST_FILENAME} !-d\nRewriteRule . /blog/index.php [L]\n</IfModule>\n# END WordPress\n";
    
    if (file_put_contents($htaccess_path, $htaccess_content)) {
        log_action(".htaccess created with WordPress rewrite rules.", "success");
    } else {
        log_action("Failed to create .htaccess. Please check directory permissions.", "error");
    }
}
